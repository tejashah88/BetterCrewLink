...
// HACK: Inlining the Audio Worklet here since Webpack + Electron is hard to wrangle with
// Specifically, its tricky to specify the code file for the audio worklet from a path since
// Electron transpiles it all in an unknown way.
const VOLUME_LIMITER_CODE = `
// Default constants
...
registerProcessor('volume-limiter-processor', LimiterProcessor);
`;
			...
			function createPeerConnection(peer: string, initiator: boolean, client: Client) {
				...
				connection.on('stream', async (stream: MediaStream) => {
					...
					try {
						...
						const workletBlob = new Blob([VOLUME_LIMITER_CODE], { type: 'text/javascript' });
